...
# 采用kubesec工具针对yaml文件漏洞扫描

安装kubesec工具，确认操作系统内核是amd还是arm，输入
...
```bash
x86_64  #x86_64，x64，AMD64基本上是同一个东西
```

在下载网站选择对应的安装包下载，https://github.com/controlplaneio/kubesec/releases  复制  [kubesec_linux_amd64.tar.gz](https://github.com/controlplaneio/kubesec/releases/download/v2.11.4/kubesec_linux_amd64.tar.gz)下载链接，下载

```bash
wget https://github.com/controlplaneio/kubesec/releases/download/v2.11.4/kubesec_linux_amd64.tar.gz
```

tar解压，并把kubesec二进制文件移动到/usr/bin下

```bash
tar -vxf kubesec_linux_amd64.tar.gz
...
首先应注意到第一点-30分，表示比较严重问题，这里selector表示privileged== true有问题，即原yaml文件中 securityContext:privileged: true 这项有问题，不应为true，应修改为false，reason即原因解释为设置为true后，这个容器几乎可以不受限制地访问主机，这是很危险的行为
